import { Flight, AirportConfig, RunwayUsage, GateUsage, ScheduleResult, TimelineEvent, OperationType } from './types.js';
...
function getRunwayBuffer(op1: OperationType, op2: OperationType, config: AirportConfig): number {
  if (op1 === op2) {
...
export function generateTimeline(scheduledFlights: Flight[], config: AirportConfig): TimelineEvent[] {
  const events: TimelineEvent[] = [];

  for (const f of scheduledFlights) {
    if (!f.scheduledTime || !f.assignedRunway || !f.assignedGate) continue;
...
export function scheduleAirport(
  ...
  const scheduledFlights: Flight[] = [];
  const unscheduledFlights: { flight: Flight; reason: string }[] = [];
  ...
  const runwayReservations = new Map<string, Array<{ flightNumber: string; opType: OperationType; time: Date }>>();
  for (let i = 1; i <= config.runwayCount; i++) {
    runwayReservations.set(`Runway ${i}`, []);
  }

  const gateReservations = new Map<string, Array<{ flightNumber: string; start: Date; end: Date }>>();
  for (let i = 1; i <= config.gateCount; i++) {
    gateReservations.set(`Gate ${i}`, []);
  }

  const crewReservations = new Map<number, Array<{ flightNumber: string; start: Date; end: Date }>>();
  ...
  const hasCycle = (flightNum: string, visited: Set<string>, stack: Set<string>): boolean => {
    if (stack.has(flightNum)) return true;
    if (visited.has(flightNum)) return false;
  ...
  const cyclicFlightNumbers = new Set<string>();
  ...
  const scheduledTimeMap = new Map<string, Date>();
  ...
  while (remaining.length > 0) {
    ...
    while (tSearch <= horizonEnd) {
      ...
      let selectedRunway: string | undefined;
      ...
      let linkedArrivalDep: Flight | undefined;
      ...
      let selectedGate: string | undefined;
      ...
      let selectedCrewId: number | undefined;
...
export function analyzeBottlenecks(
  ...
  const getEndTime = (f: Flight): Date => {
    if (!f.scheduledTime) return new Date(0);
