...
function isTimeInInterval(time, startStr, endStr) {
...
async function resetSimulatedClock() {
    try {
        ...
        const queue = JSON.parse(queueRes.contents?.[0]?.text || '[]');
        if (queue.length > 0) {
...
async function fetchAirportState() {
    try {
        ...
        lastStatusText = statusResult.content?.[0]?.text || 'No status available.';
        const availRes = await client.readResource({ uri: 'resources://availability' });
        lastAvail = JSON.parse(availRes.contents?.[0]?.text || '{}');
        const timelineRes = await client.readResource({ uri: 'flights://timeline' });
        lastTimeline = JSON.parse(timelineRes.contents?.[0]?.text || '[]');
    }
    catch (err) {
...
function getLeftLines() {
    ...
    if (runwayIds.length === 0) {
        ...
        for (const rwId of runwayIds) {
            const activeFlight = runways[rwId].find((f) => {
    ...
    if (gateIds.length === 0) {
        ...
        for (const gateId of gateIds) {
            const active = gates[gateId].find((f) => isTimeInInterval(simulatedTime, f.occupiedFrom, f.occupiedTo));
            if (active) {
    ...
    if (crewIds.length === 0) {
        ...
        for (const crewId of crewIds) {
            const active = crews[crewId].find((f) => isTimeInInterval(simulatedTime, f.busyFrom, f.busyTo));
            if (active) {
    ...
    const countLines = statusLines.filter((l) => l.trim().startsWith('-'));
...
async function submitScenarioFlights(scenarioFlights) {
    const queueRes = await client.readResource({ uri: 'flights://queue' });
    const queue = JSON.parse(queueRes.contents?.[0]?.text || '[]');
    const existingFlightNumbers = new Set(queue.map((f) => f.flightNumber.toUpperCase()));
    ...
    for (const f of scenarioFlights) {
        ...
        const mappedDeps = (f.dependencies || []).map((dep) => {
        ...
        switch (choice.trim()) {
            case '1': {
                ...
                try {
                    ...
                    logResponse(res.content?.[0]?.text || 'Submitted.');
            ...
            case '2': {
                ...
                try {
                    ...
                    logResponse(res.content?.[0]?.text || 'Refreshed.');
                    await resetSimulatedClock();
                    await fetchAirportState();
                    drawScreen();
                }
            ...
            case '3': {
                ...
                try {
                    ...
                    logResponse(res.content?.[0]?.text || 'Cancelled.');
                    await fetchAirportState();
                    drawScreen();
                }
                catch (e) {
            ...
            case '4': {
                ...
                try {
                    ...
                    logResponse(res.content?.[0]?.text || 'No bottlenecks.');
                }
                catch (e) {
                    logResponse(`${FG_RED}Error: ${e.message}${RESET}`);
                }
            ...
            case '5': {
                ...
                try {
                    ...
                    logResponse(res.contents?.[0]?.text || 'Empty Queue.');
                }
                catch (e) {
                    logResponse(`${FG_RED}Error: ${e.message}${RESET}`);
                }
            ...
            case '7': {
                ...
                try {
                    ...
                    logResponse(res.contents?.[0]?.text || 'No data.');
                }
                catch (e) {
                    logResponse(`${FG_RED}Error: ${e.message}${RESET}`);
                }
            ...
            case '8': {
                ...
                try {
                    ...
                    logResponse(schedRes.content?.[0]?.text || 'Morning Rush generated.');
                    await resetSimulatedClock();
                    await fetchAirportState();
                    drawScreen();
                }
            ...
            case '9': {
                ...
                try {
                    ...
                    logResponse(schedRes.content?.[0]?.text || 'Heavy Hauler generated.');
                    await resetSimulatedClock();
                    await fetchAirportState();
                    drawScreen();
                }
            ...
            case '10': {
                ...
                try {
                    ...
                    logResponse(schedRes.content?.[0]?.text || 'Connecting Flight generated.');
                    await resetSimulatedClock();
                    await fetchAirportState();
                    drawScreen();
                }
