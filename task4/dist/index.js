...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'submit_flight': {
                const { flightNumber, operationType, priority, plannedTime, dependencies, runwayRequirement, gateRequirement } = args;
            ...
            case 'get_airport_status': {
                ...
                let occupiedGates = 0;
                const activeGateAssignments = [];
                let occupiedRunways = 0;
                const activeRunwayAssignments = [];
                let activeGroundCrew = 0;
                for (const f of scheduledFlights) {
                    if (!f.scheduledTime || !f.assignedGate || !f.assignedRunway)
                        continue;
            ...
            case 'cancel_flight': {
                const { flightNumber, reason = 'Cancelled by air traffic control' } = args;
                if (!flights.has(flightNumber)) {
                    return {
